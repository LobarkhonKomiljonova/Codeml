...
$db = new database();  
 ?>
          <?php 
                 global $currentPage;
                 global $previous;
                $query="SELECT Count(id) AS NumberOfLessons FROM php;";
                $run=$db->select($query);
...
?>
 ...
 <div class="row">
   ...
   <?php     
    ...
    $query="SELECT lesson_tag FROM php WHERE id=$id";  
...
<h2><?php echo $row['lesson_tag'];?> </h2>
   ...
   <?php 
     $query="SELECT lesson_image FROM php WHERE id=$id";  
       $lesson=$db->select($query);
while($row=$lesson->fetch_array()):?> 
  ...
  <div class="jumbotron">
    <?php     
    $query="SELECT lesson_title FROM php WHERE id=$id";  
       $lesson=$db->select($query);
while($row=$lesson->fetch_array()):?> 
...
<?php 
     $query="SELECT lesson_content FROM php WHERE id=$id";  
       $lesson=$db->select($query);
while($row=$lesson->fetch_array()):?> 
  ...
  <p>This is another text.</p> 
     ...
     <ul class="pager" >
         ...
         <li class="previous" ><a href="phplesson.php?id4=<?php echo --$GLOBALS['previous'];?>">Previous</a></li>
         <li class="next"><a href="phplesson.php?id4=<?php echo $GLOBALS['currentPage'];?>">Next Chapter</a></li>
     </ul>
     </div>
